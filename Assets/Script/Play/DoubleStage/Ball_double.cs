...
class Ball_double : MonoBehaviour
{
    // ボールの移動の速さを指定する変数
   ...
   void FixedUpdate()
    {
        ...
        //     Vector2 newVelocity = velocity.normalized * speed * sqrt2;
        ...
        if (Mathf.Abs(rvelocity.x) < minSpeed )
        {
            ...
            float newx = rvelocity.x + Mathf.Sign(rvelocity.x)*minSpeed/10;
        ...
        else if (Mathf.Abs(rvelocity.y) < minSpeed )
        {
            ...
            float newy = rvelocity.y + Mathf.Sign(rvelocity.y)*minSpeed/10;
